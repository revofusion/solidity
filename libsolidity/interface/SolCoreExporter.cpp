			...
			if (hasIndexed)
			{
				if (!activeCompilerStack)
					throw UnsupportedSolCore(
						...
						"require an explicit codegen identity.");

				// Both pipelines are measured (2026-07-25, real EVM; see
				// EVAL_ORDER_TABLE.md row "emit with >=1 INDEXED parameter").
				if (activeCompilerStack->viaIR())
					// IRGeneratorForStatements evaluates ALL emit arguments in
					// source order, indexed or not: emitTwoIndexed topics
					// (1,2), emitMixed a=0,b=1,c=2,d=3. The permutation is the
					// identity, so this is the non-indexed capture verbatim.
					for (size_t i = 0; i < call.arguments().size(); ++i)
						captureArgument(i);
				else
				{
